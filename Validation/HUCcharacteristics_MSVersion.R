...
for(i in 1:length(ListRegions)){
  ...
  #combine these
  if(i==6){
    df_matching<-data.frame(cbind(df_PRECIPx$FeatureID, df_PRECIPx$PrecipV, df_TEMPx[,1:12], df_LATx$LatV))
    colnames(df_matching)<-c("FeatureID", "PrecipV", colnames(df_TEMPx[,1:12]), "LatV")
    df<-merge(df_NLCDx, df_matching, by.x="COMID", by.y="FeatureID", all.x=T)
  } else {
    df<-cbind(df_NLCDx, df_PRECIPx$PrecipV, df_TEMPx[,1:12], df_LATx$LatV) #only if cols match
    colnames(df)[38:51]<-c("PrecipV", colnames(df_TEMPx[,1:12]), "LatV") #change last two colnames
  }     
  fiName<-paste("Region", ListRegions[i], sep="") #make table name
  assign(fiName, df) #name table for region
  write.csv(df, paste("combined2/", fiName, ".csv", sep=""), row.names=F)
  ...
  for(z in 1:12){
    df[,(51+z)]<-df$CatAreaSqKm*(df[,(38+z)]/100) #area weighted temp
    Tmaxdf[1,z]<-sum(df[,(51+z)])/sum(df$CatAreaSqKm)
  }
  colnames(df)[52:63]<-c("Tmax01", "Tmax02", "Tmax03", "Tmax04", "Tmax05", "Tmax06", 
                         "Tmax07", "Tmax08", "Tmax09", "Tmax10", "Tmax11", "Tmax12")
