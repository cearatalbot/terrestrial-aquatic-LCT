######Function for generating weather data TO COMPARE TO BUTMAN####

###STOP: this is the file for version 2 of the weather generator
# this version uses sine waves for seasonal fits for PRECIP
# we IMPORT tmax data and use it for all calcs.
#library packages required
library(sirad)
source("/Users/cearatalbot/Rcode/TerrAqModel/RunningSipnetCode/evapFunc1.R")
#daily precip
#monthly temperature
#PAR
#VPD
...
genWeather2<-function(annPpt, fracWet, pptSeason, corrT, Tmaxdf, lat, tempSeason, corrV){
  ...
  for(t in 1:12){
    for(z in 1:12){
      tempDF[z,1]<- paste(z)
      monthMeanT<-Tmaxdf[1,z]
      tempDF[z,3]<-monthMeanT
      tempDF[z,4]<-monthMeanT #redundant but needed for next calcs
    ...
    newTemp[,1]<-rep(Tmaxdf[1,7]) #rep july temp (old Tmax)
    newTemp[,2]<-seq(1,tempDF$ndays[t], by=1) #generate month days
    newTemp[,3]<-rep(Tmaxdf[1,t], tempDF$ndays[t]) #rep mean monthly temp from Tmaxdf input for the number of days in each month
  ...
  ######PAR######
  #could make a relationship between precip and clouds
  ...
  #Calculate air VPD (from https://getpulse.co/blog/vpd)
  #SVP = 610.78 x e^(T / (T +238.3) x 17.2694)) #then divide by 1000 to get kPa
  #e is a mathematical constant called Euler's Number, approximately equal to 2.71828
  #SVP x (1 – RH/100) = VPD
  #a better source for VPD equations... http://www.fao.org/3/X0490E/x0490e07.htm
  #vp = RH/100 * ((SVPmax_kPa + SVPmin_kPa)/2)
  dfVPD<-data.frame(matrix(ncol=9, nrow=0))
  colnames(dfVPD)<-c("Month", "Day", "DOY", "VPDmin_kPa", "VPDmax_kPa", "VPD_meanKpa", "SVP_meanKpa" , "VP_meanKpa", "lakeEvap")
  ...
  for(x in 1:12){
    ...
    colnames(dfVPD1)<-c("Month", "Day", "DOY", "VPDmin_kPa", "VPDmax_kPa", "VPD_meanKpa", "SVP_meanKpa" , "VP_meanKpa", "lakeEvap")
    dfVPD1[,1]<-rep(x)
  ...
  df<-as.data.frame(cbind(YearPrecip$Month, YearPrecip$Day, YearPrecip$Precip, YearPrecip$meanAppt , YearTemp$MeanTemp, YearPAR$DOY, YearPAR$PAR, dfVPD$VPD_meanKpa, dfVPD$lakeEvap, SoilTemp$SoilTemp))
  colnames(df)<-c("Month", "Day", "Precip", "AnnPpt", "MeanTemp", "DOY", "PAR", "VPD", "Evap", "SoilTemp")
  
  #df$Precip<-round(as.numeric(df$Precip), digits=4)
  #df$Evap<-round(as.numeric(df$Evap), digits=4)
  #df$VPD<-round(as.numeric(df$VPD), digits=4)
  #df$MeanTemp<-round(as.numeric(df$MeanTemp), digits=3)
  return(df)}